...
function setup() {
  ...
  async function getMap(level) {
    const response = await fetch('/map/'+ level + '/');
    ...
    if (keyCode === ENTER) {
      ...
      for (var i = 0; i < options.length; i++) {
        ...
        if (grid[tmpX][tmpY].goal == 'doors') {
          ...
          var data = {"condition":condition, "userid": uid, "episode":episode, "target":"door", "target_pos":targetPos.toString(),
          ...
          if (countPress == 5) {
            ...
            var targetPos = [curX, curY];
            var data = {"condition":condition, "userid": uid, "episode":episode, "target":"green_victim", "target_pos":targetPos.toString(),
                  "num_step":targetSteps, "time_spent": display.textContent, "trajectory":traces.join(";")};
            writeData(data);
            targetSteps = 0;
            traces = [];
          ...
          if (countPress == 10) {
            ...
            document.getElementById('goal').innerHTML = 'Points: ' + rescue.toString();
            var targetPos = [curX, curY];
            var data = {"condition":condition, "userid": uid, "episode":episode, "target":"yellow_victim", "target_pos":targetPos.toString(),
                  "num_step":targetSteps, "time_spent": display.textContent, "trajectory":traces.join(";")};
            writeData(data);
            targetSteps = 0;
            traces = [];
